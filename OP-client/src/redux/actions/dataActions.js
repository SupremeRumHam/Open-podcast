...
export const submitComment = (updatesId, commentData) => (dispatch) => {
  axios
    .post(`/update/${updatesId}/comments`, commentData)
    .then((res) => {
      dispatch({
        type: SUBMIT_COMMENT,
        payload: res.data
...
export const deleteUpdate = (updatesId) => (dispatch) => {
  axios
    .delete(`/update/${updatesId}`)
    .then(() => {
      dispatch({ type: DELETE_UPDATE, payload: updatesId });
    })
    .catch((err) => console.log(err));
};
